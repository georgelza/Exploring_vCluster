## Single Node vCluster, on Docker with local PV/PVC.
...
vcluster create my-vc1 --values vcluster_1.yaml
...
```bash
kubectl get pods -n ingress-traefik -w
# or for more detail
...
```bash
docker inspect vcluster.cp.my-vc1 | grep IPAddress
```

10. Test it (replace with your ip and nodePort from step 6, use the http port, not https)

```bash
docker exec vcluster.cp.my-vc1 curl -s -H "Host: my-vc.local" http://<container_ip>:<nodeport>
# docker exec vcluster.cp.my-vc2 curl -s -H "Host: my-vc.local" http://172.19.0.2:32716

```

docker exec vcluster.cp.my-vc1 curl -s -H "Host: my-vc.local" http://172.19.0.2:30323
